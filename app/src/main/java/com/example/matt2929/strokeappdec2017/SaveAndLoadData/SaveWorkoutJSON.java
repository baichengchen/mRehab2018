...
public class SaveWorkoutJSON {
	...
	public void addNewWorkout(String WorkoutName, String Hand, float Duration, float Accuracy, int Reps) {
		...
		String fileName = WorkoutData.UserName + "_" + WorkoutName + "_" + currentCalendar.get(Calendar.YEAR) + "~" + (currentCalendar.get(Calendar.MONTH)+1) + "~" + currentCalendar.get(Calendar.DAY_OF_MONTH) + "_[" + currentCalendar.get(Calendar.HOUR_OF_DAY) + "h~" + currentCalendar.get(Calendar.MINUTE) + "m" + currentCalendar.get(Calendar.SECOND) + "]_JSON.json";
		String output = "";
